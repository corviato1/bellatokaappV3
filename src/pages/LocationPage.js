...
const LocationPage = () => {
  ...
  const currentPlant = plants.find(p => p.currentLocation === locationId && p.status === 'active');
  ...
  const hideSchedule = filters.stats || filters.nutrients || filters.pestManagement || filters.images;

  const getStatsMetrics = () => {
    if (locationEntries.length === 0) return null;
    
    const latestEntry = locationEntries[0];
    const previousEntry = locationEntries.length > 1 ? locationEntries[1] : null;
  ...
  const metricsTemplate = [
    { key: 'temperature', label: 'Temperature', unit: '°F', isComplex: true, dayNight: true },
    { key: 'humidity', label: 'Humidity', unit: '%', isComplex: true, dayNight: true },
    ...
    { key: 'lightIntensity', label: 'Light Intensity', unit: 'PPFD', isComplex: false },
    ...
    <div className="location-page">
      ...
      {currentPlant && (
        <div 
          ...
          onClick={() => navigate(`/plant/${currentPlant.id}-${currentPlant.strain}/stats`)}
        >
            ...
            <div className="schedule-list">
              {weekDays.map((day, idx) => {
                ...
                return (
                  <div key={idx}>
                    <div 
                      className={`schedule-day ${isSelected ? 'active' : ''} ${dayEntries.length > 0 ? 'has-entries' : ''}`}
                              ...
                              <div key={entryIdx}>
                                ...
                                {entry.stats?.notes && (
                                  <div className="day-task">
                                    ...
                                    <div className="task-notes">{entry.stats.notes}</div>
                                  </div>
                                )}
                              </div>
                            ))}
            ...
            {filters.stats && (
              <div className="static-section stats-panel">
                {(() => {
                  ...
                  if (!statsData || Object.keys(statsData.current).length === 0) {
                    ...
                    <>
                      <div className="stats-header">
                        ...
                        {statsData.previousWeek && (
                          <span className="compare-label">vs Week {statsData.previousWeek}</span>
                        )}
                      </div>
                      ...
                      <div className="metrics-grid">
                        {metricsTemplate.map(metric => {
                          ...
                          if (metric.dayNight && typeof currentVal === 'object') {
                            const dayTrend = getTrend(currentVal.day, previousVal?.day);
                            const nightTrend = getTrend(currentVal.night, previousVal?.night);
                            return (
                              <div key={metric.key} className="metric-card complex">
                                <div className="metric-label">{metric.label}</div>
                                <div className="metric-row">
                                  <span className="period">Day:</span>
                                  <span className="metric-value">
                                    {currentVal.day}{metric.unit}
                                    {dayTrend && <span className={`trend ${dayTrend}`}>{dayTrend === 'up' ? '↑' : dayTrend === 'down' ? '↓' : '→'}</span>}
                                  </span>
                                  {previousVal?.day !== undefined && (
                                    <span className="metric-previous">was {previousVal.day}{metric.unit}</span>
                                  )}
                                </div>
                                <div className="metric-row">
                                  <span className="period">Night:</span>
                                  <span className="metric-value">
                                    {currentVal.night}{metric.unit}
                                    {nightTrend && <span className={`trend ${nightTrend}`}>{nightTrend === 'up' ? '↑' : nightTrend === 'down' ? '↓' : '→'}</span>}
                                  </span>
                                  {previousVal?.night !== undefined && (
                                    <span className="metric-previous">was {previousVal.night}{metric.unit}</span>
                                  )}
                                </div>
                              </div>
                            );
                          ...
                          return (
                            <div key={metric.key} className="metric-card">
                              ...
                              {previousVal !== undefined && (
                                <div className="metric-previous">was {previousVal}{metric.unit}</div>
                              )}
                            </div>
                          );
                        })}
                ...
                <div className="nutrients-grid">
                  {(() => {
                    ...
                    return allNutrients.map((nutrient, idx) => (
                      <div 
                        key={idx} 
                        className={`nutrient-item ${selectedNutrient === nutrient.image ? 'selected' : ''}`}
              ...
              <div className="static-section">
                ...
                <div className="pest-grid">
                  {(() => {
                    ...
                    if (allPest.length === 0) {
                      return <p className="no-data">No pest management recorded</p>;
                    }
                    return allPest.map((item, idx) => (
                      <div key={idx} className="pest-item">
                        ...
                        <span>{item.name}</span>
                        {item.link && (
                          <a href={item.link} target="_blank" rel="noopener noreferrer">Info</a>
                        )}
                      </div>
                    ));
                  })()}
